...
| A-13 | **Reset v2 connective copy.** Chip labels, at-rest labels, the demand hypothesis, the "good V1 language" lines and the authority chain are verbatim from Reset v2; the other four hypotheses reuse the Script's approved acknowledgements. Written for this preview and needing sign-off: left-rail item names' targets, refinement question titles and `Why it matters` lines, at-rest tile summaries, `Sample shown:` / `Complete sample loaded` / `Edit signal` microcopy, the composer placeholder and hint. | Reset v2 §5–§8 supply the model but not every string. | Edit `src/features/ai-card-app/content/appCopy.ts`; content tests keep the guardrails. |
| A-14 | **Reset v2 base fixtures.** With a challenge but no business type: demand → MSP demand sample (named in Reset §10), capacity → services coordination sample, the other three chips → the transparent core model. | Reset §9 "primaryChallenge alone resolves to an honest base fixture"; no chip-specific fixtures exist for fragmented/website/measurement. | Add fixtures and edit `BASE_PATH_BY_CHALLENGE`. |
| A-15 | **Reset v2 route** `/ai-card-app/` is a preview name only; the v1 wizard stays at `/ai-card/` as the rollback baseline. | Reset §10 "Do not modify `/ai-card/` in place without a reviewed preview". | Rename folders once Chris decides (D-13). |
...
| C-06 | Reset v2 retires the Wireframe's welcome → Q1…Q5 → assembly topology and the Copy Deck's `Build my AI Card` start screen inside the Card. | Reset v2 §1/§4 vs Wireframe W01–W04, Copy Deck §18 | Reset v2 wins (Chris's explicit written decision, Brief §2). The homepage hero CTA text `Build my AI Card` is unchanged and now opens the app-first surface. |
